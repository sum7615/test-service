...
public interface ITestUserMapRep extends JpaRepository<TestUserMap,Long> {
    ...
    @Query("Select t from TestUserMap t where lower(t.assignedTo)=lower( :userName) and t.isActive=true")
    Optional<List<TestUserMap>> getTestByUserName(@Param("userName") String userName);

    @Query("Select t from TestUserMap t where lower(t.assignedTo)=lower( :userName) and t.testId.endTime > CURRENT_TIMESTAMP and t.testId.startTime > CURRENT_TIMESTAMP  and t.isActive=true")
    ...
    @Query("Select t from TestUserMap t where lower(t.assignedTo)=lower( :userName) and t.testId.id= :testId and t.isActive=true and t.isCompleted=false order by assignedAt desc")
    List<TestUserMap> getByUserAndTest(@Param("userName") String userName, @Param("testId") Long testId);
}
