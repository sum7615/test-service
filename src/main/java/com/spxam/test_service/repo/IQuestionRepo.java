...
public interface IQuestionRepo extends JpaRepository<Question, Long> {
    ...
    @Query("SELECT q FROM Question q WHERE q.bankId.id = :questionBankId AND q.isActive = true")
    List<Question> findByQuestionBank(@Param("questionBankId") Long questionBankId);
