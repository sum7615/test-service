...
public class QuestionControllerValidator {
   ...
   private final ITestRepo iTestRepo;
    public ValidationResult validate(CreateQuestionPayload payload){
        ...
        CommonUtil.validateMandatory(payload.level(),"Level",errors);
	...
	public ValidationResult validateUpdate(UpdateQuestionPayload payload) {
		 List<String> errors = new java.util.ArrayList<>();
	        ...
	        CommonUtil.validateMandatory(payload.type(),"Type",errors);
	        CommonUtil.validateMandatory(payload.questionBankId(),"Question Bank",errors);
	        CommonUtil.validateMandatory(payload.id(),"Question",errors);
	       ...
	       if(questIonBank.isEmpty()){
	        ...
	        if(questIonBank.isPresent()){
	            if(!questIonBank.get().getCreatedBy().equals(payload.userName())){
	                errors.add("User not authorized to add question in this question bank");
	            }
	        }
	...
	public ValidationResult validateDelete(DeleteQuestionPayload payload) {
			List<String> errors = new java.util.ArrayList<>();
